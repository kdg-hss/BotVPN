...
async function renewssh(username, exp, limitip, serverId) {
  ...
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
      ...
      const param = `:2269/renewssh?user=${username}&exp=${exp}&iplimit=${limitip}&auth=${auth}`;
...
async function renewvmess(username, exp, quota, limitip, serverId) {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
        ...
        const domain = server.domain;
        const auth = server.auth;
        const param = `:2269/renewvmess?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
        const url = `http://${domain}${param}`;
        axios.get(url)
          .then(response => {
            if (response.data.status === "success") {
  ...
  async function renewvless(username, exp, quota, limitip, serverId) {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
        ...
        const domain = server.domain;
        const auth = server.auth;
        const param = `:2269/renewvless?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
        const url = `http://${domain}${param}`;
        axios.get(url)
          .then(response => {
            if (response.data.status === "success") {
  ...
  async function renewtrojan(username, exp, quota, limitip, serverId) {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
        ...
        const domain = server.domain;
        const auth = server.auth;
        const param = `:2269/renewtrojan?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
        const url = `http://${domain}${param}`;
        axios.get(url)
          .then(response => {
            if (response.data.status === "success") {
  ...
  async function renewshadowsocks(username, exp, quota, limitip, serverId) {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM Server WHERE id = ?', [serverId], (err, server) => {
        ...
        const domain = server.domain;
        const auth = server.auth;
        const param = `:2269/renewshadowsocks?user=${username}&exp=${exp}&quota=${quota}&iplimit=${limitip}&auth=${auth}`;
        const url = `http://${domain}${param}`;
        axios.get(url)
          .then(response => {
            if (response.data.status === "success") {
